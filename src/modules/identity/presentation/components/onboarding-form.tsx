...
export function OnboardingForm({ locale, initialName }: { locale: "es" | "en"; initialName: string }) {
  ...
  function submit(formData: FormData) { setError(undefined); formData.set("locale", locale); startTransition(async () => { const result = await completeOnboardingAction(formData); if (!result.ok) setError(result.error); else router.push(result.data.redirectTo as Route); }); }
