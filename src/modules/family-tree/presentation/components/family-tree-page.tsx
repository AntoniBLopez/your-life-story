...
export function FamilyTreePage({ locale, people, relationships }: Props) {
  ...
  const t = locale === "es" ? { eyebrow: "Herramienta complementaria", title: "Mi familia", intro: "Construye tu árbol familiar a tu ritmo. Empieza por ti y conecta a las personas que importan.", person: "Añadir persona", edit: "Editar datos", details: "Datos de la persona", close: "Cerrar", link: "Crear vínculo", name: "Nombre completo", birth: "Nacimiento", death: "Fallecimiento", country: "País de nacimiento", city: "Ciudad de nacimiento", me: "Esta persona soy yo", save: "Guardar persona", update: "Guardar cambios", origin: "Origen", target: "Destino", type: "Relación", parent: "es progenitor de", partner: "es pareja de", sibling: "es hermano/a de", empty: "Empieza contigo", emptyBody: "Añádete como persona principal y después conecta el resto de tu historia familiar.", example: "Así podría verse tu árbol familiar", mapHelp: "Arrastra, acerca y explora el árbol. Los parentescos se calculan respecto a la persona marcada como tú.", relation: "Relación", linked: "Vínculo creado", import: "Importar GEDCOM", export: "Exportar GEDCOM", file: "Archivo GEDCOM" } : { eyebrow: "Companion tool", title: "My family", intro: "Build your family tree at your own pace. Start with yourself and connect the people who matter.", person: "Add person", edit: "Edit details", details: "Person details", close: "Close", link: "Create relationship", name: "Full name", birth: "Birth", death: "Death", country: "Country of birth", city: "City of birth", me: "This person is me", save: "Save person", update: "Save changes", origin: "From", target: "To", type: "Relationship", parent: "is parent of", partner: "is partner of", sibling: "is sibling of", empty: "Start with yourself", emptyBody: "Add yourself as the main person and then connect the rest of your family story.", example: "This is how your family tree could look", mapHelp: "Drag, zoom and explore the tree. Relationships are calculated from the person marked as you.", relation: "Relationship", linked: "Relationship created", import: "Import GEDCOM", export: "Export GEDCOM", file: "GEDCOM file" };
...
function buildGraph(people: FamilyPerson[], relationships: FamilyRelationship[], subjectId: string | undefined, locale: "es" | "en") {
  ...
  const nodes: Node[] = [];
  [...groups.entries()].forEach(([generation, group]) => group.forEach((person) => nodes.push({ id: person.id, position: { x: 80 + (xByPerson.get(person.id) ?? 0) - minimumX, y: 110 + (generation + 2) * 190 }, style: { width: 224, ...(person.isSubject ? { border: "2px solid #3d654c", background: "#eef5ec", boxShadow: "0 0 0 4px #dbe8d8, 0 12px 24px #244a3630" } : {}) }, data: { label: <FamilyNode person={person} label={relationToSubject(person.id, subjectId, relationships, people, locale)} locale={locale} /> } })));
