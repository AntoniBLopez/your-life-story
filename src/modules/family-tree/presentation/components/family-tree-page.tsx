...
import { Cake, CalendarDays, Download, HeartHandshake, Mail, MapPin, PartyPopper, Upload, UserPlus, UsersRound } from "lucide-react";
...
import { childrenOf, familyAvatarUrl, hasFamilyAvatar, relationToSubject, resolveParentSlots, type FamilyPerson, type FamilyRelationship } from "@/modules/family-tree/domain/family-graph";
...
export function FamilyTreePage({ locale, people, relationships, readOnly, embedded, youPersonId, presets = [], calendar, calendarNotice }: Props) {
  ...
  const selectedChildCount = useMemo(
    () => (selectedPerson ? childrenOf(selectedPerson.id, relationships).length : 0),
