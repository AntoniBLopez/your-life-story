...
export async function searchUserStory(userId: string, query: string) {
  ...
  return {
    entries: entries.filter((entry) => matches([entry.title, entry.narrative, entry.difficulty, entry.learning, entry.transformation, entry.lifeArea, ...(entry.lifeAreas ?? []), entry.startDate, entry.endDate, ...entry.tags])),
